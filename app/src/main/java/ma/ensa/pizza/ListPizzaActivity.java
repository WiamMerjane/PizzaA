...
import ma.ensa.pizza.adapter.PizzaAdapter;
import ma.ensa.pizza.classes.Produit;
import ma.ensa.pizza.service.ProduitService;
...
//        ps.create(new Produit("BARBECUED CHICKEN PIZZA", 3, R.drawable.pizza1, "35 min", "- 2 boneless skinless chicken breast halves (6 ounces each)\n- 1/4 teaspoon pepper\n- 1 cup barbecue sauce, divided\n- 1 tube (13.8 ounces) refrigerated pizza crust\n- 2 teaspoons olive oil\n-2 cups shredded Gouda cheese\n-1 small red onion, halved and thinly sliced\n-1/4 cup minced fresh cilantro","So fast and so easy with refrigerated pizza crust, these saucy, smoky pizzas make quick fans with their hot-off-the-grill, rustic flavor. They're perfect for spur-of-the-moment cookouts and summer dinners on the patio. —Alicia Trevithick, Temecula, California",
//                "STEP 1:\n\n Sprinkle chicken with pepper; place on an oiled grill rack over medium heat. Grill, covered, until a thermometer reads 165°, 5-7 minutes per side, basting frequently with 1/2 cup barbecue sauce during the last 4 minutes. Cool slightly. Cut into cubes.\n\nSTEP 2:\n\n Divide dough in half. On a wellgreased large sheet of heavy-duty foil, press each portion of dough into a 10x8-in. rectangle; brush lightly with oil. Invert dough onto grill rack; peel off foil. Grill, covered, over medium heat until bottom is lightly browned, 1-2 minutes.\n\nSTEP 3:\n\n Remove from grill. Spread grilled sides with remaining barbecue sauce. Top with cheese, chicken and onion. Grill, covered, until bottom is lightly browned and cheese is melted, 2-3 minutes. Sprinkle with cilantro. Yield: 2 pizzas (4 pieces each)."));
        ...
        ps.create(new Produit("BACON CHEESEBURGER PIZZA",2,R.mipmap.pizza8,"20min",
...
////                intent.putExtra("itemId", Integer.parseInt(idd.getText().toString()));

                TextView id = view.findViewById(R.id.id);

                // Récupérez la pizza sélectionnée
                Produit selectedPizza = ps.findById(Integer.parseInt(id.getText().toString()));

                // Lancez l'activité PizzaDetailsActivity en passant les informations nécessaires
                Intent intent = new Intent(ListPizzaActivity.this, MainActivity.class);
                intent.putExtra("pizzaName", selectedPizza.getNom());
                intent.putExtra("pizzaPhoto", selectedPizza.getPhoto());
                intent.putExtra("pizzaDuration", selectedPizza.getDuree());
                intent.putExtra("pizzaDescription", selectedPizza.getDescription());
                intent.putExtra("pizzaIngredients", selectedPizza.getDetaisIngred());
                intent.putExtra("pizzaPreparation", selectedPizza.getPreparation());
